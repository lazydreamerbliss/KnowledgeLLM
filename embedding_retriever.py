...
class EmbeddingRetriever:
    ...
    def extract_features_and_build_index(self, documents: List[str], output_filename: str,
        ...
        self.embeddings = np.asarray(
            [self.model.encode(x) for x in tqdm(documents, desc='Extracting features from documents...')])
        self.index = IndexFlatIP(self.embeddings.shape[1])
        self.index.add(self.embeddings)
        pickle.dump({'documents': self.documents, 'embeddings': self.embeddings, 'index': self.index, },
