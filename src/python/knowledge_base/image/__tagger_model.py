"""Copied from https://github.com/fpgaminer/joytag

All credit goes to fpgaminer.
"""
...
class TaggerModel(nn.Module):
    ...
    @staticmethod
    def load_model(path: Path | str, device: str | None = None) -> 'TaggerModel':
        """
        ...
        model_classes = TaggerModel.__subclasses__()
    ...
    @staticmethod
    def from_config(config: dict) -> 'TaggerModel':
        model_classes = TaggerModel.__subclasses__()
        model_cls = next(cls for cls in model_classes if cls.__name__ == config['class'])
    ...
    def forward(self, pixel_values: torch.FloatTensor) -> torch.Tensor:
        ...
        if self.sine_positional_embeddings:
            position_embeddings = sinusoidal_position_embedding(
                W // self.patch_size, H // self.patch_size, self.hidden_size, pixel_values.dtype, pixel_values.device)
        else:
...
class CLIPLikeModel(TaggerModel):
    def __init__(
            self,
            n_tags: int,
            embedding_dim: int,
        ...
        stochastic_depth_space = torch.linspace(
            0, stochastic_depth, num_layers) if stochastic_depth is not None else None

        self.embedding_layer = CLIPEmbeddingLayer(
            embedding_dim, num_channels, image_size, patch_size, patch_dropout, good_dropout, dpn, sine_positional_embeddings)
        ...
        if use_gap_head:
            ...
            self.pooling_head = MHAPoolingHead(embedding_dim, num_attention_heads,
                                               activation_cls, out_dim, use_mha_alt, norm_qk=norm_qk)
...
class MaskedAutoEncoderViT(nn.Module):
    ...
    def forward(self, batch):
        ...
        if self.sine_positional_embeddings:
            position_embeddings = sinusoidal_position_embedding(
                W // self.patch_size, H // self.patch_size, self.embedding_dim, pixel_values.dtype, device)
            decoder_position_embeddings = sinusoidal_position_embedding(
                W // self.patch_size, H // self.patch_size, self.decoder_embedding_dim, pixel_values.dtype, device)
        else:
            position_embeddings = self.positional_embeddings(self.position_ids)
...
class PosEmbedding(nn.Module):
    ...
    def forward(self, x, width: int, height: int):
        if self.use_sine:
            position_embeddings = sinusoidal_position_embedding(
                width // self.patch_size, height // self.patch_size, self.d_model, x.dtype, x.device)
        else:
...
class ViT(TaggerModel):
    def __init__(self,
        ...
        self.pos_embedding = PosEmbedding(d_model, (image_size // patch_size) ** 2,
                                          use_sine=use_sine, patch_size=patch_size)
